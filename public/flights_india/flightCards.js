                                    ...
                                    <div class="spacing-0 flex-1 flight-info_flight-details row">
                                        <div class="col-lg-4 col-md-3 col-sm-3 col-3">
                                            <div class="flight-info_flight-details">
                                                <div class="d-flex">
                                                    <div class="d-flex me-2 flight-image"><img class="vertical-bottom" alt="Flight" src="https://static.iween.co.in/airlinelogos/${this.flight.AirlineCode}.jpg">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="d-flex flex-column flight-name"><span class="flight-company">${this.flight.AirlineName}</span><span class="flight-company-secondary text-capitalize" title="Saver">${this.flight.Supplier}</span><span class="fs-10 flight-number">${this.flight.AirlineCode}-${this.flight.FlightNumber}</span>
                                        ...
                                        <div class="col-lg-8 col-md-9 col-sm-9 col-9">
                                            <div class="row">
                                                ...
                                                <div class="d-flex justify-content-end col-lg-5 col-md-5 col-sm-5 col-4">
                                                    <div class="d-flex flex-column text-end"><span class="title fw-600">${this.flight.Destination.ArrTime}<span class="extra-days">+1</span></span><span class="fs-12">${this.flight.Destination.ArrDate}</span><span class="fs-10">${this.flight.Destination.CityName}</span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="fare-summary_container flex-1">
                                    ...
                                    <div class="fare-summary_table watermarked"
                                        ...
                                        <div class="row spacing-0 px-2 py-1 d-flex flex-column gap-2">
                                            ...
                                            ${infantCount}
                                                 <div class="d-flex justify-content-between"><span
                                                        class="fw-500">Platform Fee</span><span class="fw-600">₹${platformFee}</span></div>
                                            <div class="d-flex justify-content-between"><span
                                                        class="fw-500">Platform Tax</span><span class="fw-600">₹${calculatedTax}</span></div>
                                        </div>
                                        <div class="row spacing-0 px-2 py-1 d-flex flex-column gap-2 paymentBreak hidden">
...
async function book(index) {
    ...
    window.location.href = '/flights/book';
    // } else {
    //     Swal.close();
    //     alert("problem")
    // }
...
function setPassportIssueDateRestrictions(input) {
    // Assuming arr.Origin.DepDate is in "21 Sept 2024" format
    const depDateStr = arr.Origin.DepDate;
    ...
    // Set the max date to 10 days before the current date
    maxDate = new Date(currentDate);
    maxDate.setDate(maxDate.getDate() - 9); // Subtract 10 days
...
function setPassportExpiryDateRestrictions(input) {
    // Assuming arr.Origin.DepDate is in "21 Sept 2024" format
    const depDateStr = arr.Origin.DepDate;
    ...
    // Set the minimum date to 6 months after the current date
    minDate = new Date(currentDate);
    minDate.setMonth(minDate.getMonth() + 6); // Add 6 months to the departure date
    minDate.setDate(minDate.getDate() + 1);
    // Check if minDate is correctly calculated
    if (minDate) {
        console.log("Calculated Min Date: ", minDate); // Debugging log
        const formattedMinDate = minDate.toISOString().split('T')[0]; // Format min date as YYYY-MM-DD
        input.setAttribute('min', formattedMinDate);
        console.log('Min Date Set:', formattedMinDate); // Debugging log
    } else {
        console.error('Failed to calculate minDate');
    }
}
